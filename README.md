...
A complete, searchable **audit log** for your Shopify store. Smart Activity Tracker subscribes to every available store webhook and records each change — products, orders, customers, collections, inventory, fulfillments, discounts, themes and more — into a Postgres database, then surfaces them in an embedded admin dashboard with search, filtering and pagination.

Built on the [Shopify React Router app template](https://github.com/Shopify/shopify-app-template-react-router).

## Features

- **Real-time capture** of all common store events via webhooks (a single catch-all handler stores each event).
- **Beautiful embedded dashboard** built with Polaris web components: overview stats, a filterable activity table, and per-event detail with the full raw payload.
- **Search & filters** by free text, resource, action, user (name/email), and date range — all URL-driven and shareable.
...
This app uses **PostgreSQL** and is designed to run on [Railway](https://railway.app).

### 1. Provision the database

In your Railway project, add the **PostgreSQL** plugin. Railway exposes a `DATABASE_URL` connection string that the app reads automatically.
...
| --- | --- |
| `DATABASE_URL` | Provided by the Railway Postgres plugin (reference it via `${{Postgres.DATABASE_URL}}`). |
| `SHOPIFY_API_KEY` | Your app's client id (from `shopify.app.toml` / Partner dashboard). |
...
Railway builds using the included `Dockerfile`. On boot, `npm run docker-start` runs `prisma migrate deploy` (applies migrations, creating the `Session` and `ActivityLog` tables) then starts the server.

> **First deploy:** generate the initial Postgres migration locally once and commit it, so `migrate deploy` has something to apply:
> ```shell
> DATABASE_URL="<your-postgres-url>" npx prisma migrate dev --name init
> ```

### 4. Point Shopify at the Railway URL

Update the app URL and OAuth redirect in the Partner dashboard (and `shopify.app.toml` → `application_url` + `[auth].redirect_urls`) to your Railway domain, then `shopify app deploy` to push the updated webhook subscriptions. Reinstall the app on a dev store and store changes will start streaming into the dashboard.
